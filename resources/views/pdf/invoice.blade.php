...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .bal-label { font-size: 8px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.8px; color: #6b7280; }
        .bal-amount { font-size: 22px; font-weight: 700; color: #111827; margin-top: 2px; }
        ...
        .b-partial { background: #dbeafe; color: #1e3a8a; }
        .b-paid    { background: #dcfce7; color: #14532d; }
        ...
        .tv-green { font-weight: 600; color: #16a34a; text-align: right; }
        ...
        .balance-row td { background: #f0f9ff; color: #1e3a5f; font-weight: 700; font-size: 10px; }
        .balance-row td.tv { text-align: right; color: #1e3a5f; }
...
@php
    ...
    $bcls     = match(true) {
        str_contains($status, 'Fully')   => 'b-paid',
        str_contains($status, 'Partial') => 'b-partial',
        default                          => 'b-unpaid',
    };
@endphp
    ...
    <table class="hdr-tbl">
        <tr>
            ...
            <td class="hdr-right">
                ...
                <div class="doc-ref">Invoice # <b>{{ $invoice['public_id'] ?? '—' }}</b></div>
                <div class="bal-label">Balance Due</div>
                <div class="bal-amount">RM {{ number_format($balance, 2) }}</div>
            </td>
        </tr>
    </table>
</div>
    ...
    <table class="info-tbl">
        <tr>
            <td class="info-left">
                ...
                @else
                    <div class="cname">{{ ($invoice['manual_customer_data']['name'] ?? null) ?: 'Walk-in Customer' }}</div>
                    <div class="csub">
            ...
            <td class="info-right">
                <table class="meta-tbl">
                    <tr>
                        <td class="mk">Invoice Date :</td>
                        <td class="mv">{{ !empty($invoice['created_at']) ? \Carbon\Carbon::parse($invoice['created_at'])->format('d M Y') : '—' }}</td>
                    </tr>
                    @if(!empty($invoice['due_date']))
                    <tr>
                        <td class="mk">Due Date :</td>
                        <td class="mv">{{ \Carbon\Carbon::parse($invoice['due_date'])->format('d M Y') }}</td>
                    </tr>
                    @endif
                    @if(!empty($invoice['subject']))
                    <tr>
                        <td class="mk">Subject :</td>
                        <td class="mv">{{ $invoice['subject'] }}</td>
                    </tr>
                    @endif
                    <tr>
            ...
            <td class="totals-cell">
                <table class="totals-tbl">
                    <tr>
                        ...
                        <td class="tv">RM {{ number_format((float)($invoice['sub_total'] ?? 0), 2) }}</td>
                    </tr>
                    @if(($invoice['deposit_amount'] ?? 0) > 0)
                    <tr>
                        <td class="tk">Deposit</td>
                        <td class="tv">RM {{ number_format((float)$invoice['deposit_amount'], 2) }}</td>
                    </tr>
                    @endif
                    @if($paidAmt > 0)
                    <tr>
                        <td class="tk">Amount Paid</td>
                        <td class="tv-green">− RM {{ number_format($paidAmt, 2) }}</td>
                    </tr>
                    @endif
                    <tr class="total-row">
                        <td>Total</td>
                        <td class="tv">RM {{ number_format($totalAmt, 2) }}</td>
                    </tr>
                    @if($paidAmt > 0 && $balance > 0)
                    <tr class="balance-row">
                        <td>Balance Due</td>
                        <td class="tv">RM {{ number_format($balance, 2) }}</td>
                    </tr>
                    @endif
                </table>
            </td>
